		...
		public function getRelatedArticles($simple_blog_article_id) {
			...
			foreach($sql->rows as $row) {
				...
				if($article_info->row) {
                    ...
                    $image_size = getimagesize(DIR_IMAGE . $article_info->row['featured_image']);
                    if ($image_size[0] > $image_size[1]){
                        $image = $this->model_tool_image->resize($article_info->row['featured_image'], round(400 * ($image_size[0]/$image_size[1])), 400);
                    }elseif ($image_size[0] == $image_size[1]){
                        $image = $this->model_tool_image->resize($article_info->row['featured_image'], 400, 400);
                    }else{
                        $image = $this->model_tool_image->resize($article_info->row['featured_image'], 400, round(400 * ($image_size[0]/$image_size[1])));
                    }
