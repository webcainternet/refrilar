...
class ControllerModuleTmGoogleMap extends Controller
{
    public function index($setting)
    {
        ...
        $key = '';
        if ($setting['tm_google_map_key']) {
            $key .= 'key=';
            $key .= trim($setting['tm_google_map_key'] );
            $key .= '&amp;';
        }
        $this->document->addStyle('catalog/view/javascript/tmgooglemap/tmgooglemap.css');
        $this->document->addScript('//maps.googleapis.com/maps/api/js?' . $key . 'sensor=' . $setting['tm_google_map_sensor'] . '');
