...
class ControllerSimpleBlogArticle extends Controller {
	public function index() {
		...
		if ($this->config->has('config_simple_blog_limit')) {
			$limit = $this->config->get('config_simple_blog_limit');
		} else {
			$limit = 5;
		}
		...
		foreach($results as $result) {
			...
			if ($this->config->has('config_simple_blog_description_limit') && $this->config->get('config_simple_blog_description_limit') != ''){
				$description = utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_simple_blog_description_limit')) . '...';
			}else{
	...
	public function view() {
		...
		if($this->config->get('config_simple_blog_heading')) {
			$this->document->setTitle($this->config->get('config_simple_blog_heading'));
		} else {
			$this->document->setTitle($this->language->get('heading_title'));
		}
		...
		if($simple_blog_article_id) {
			...
			$related_product_header = $this->config->get('config_simple_blog_product_related_heading');
			if(!empty($related_product_header)) {
				$data['text_related_product'] = $this->config->get('config_simple_blog_product_related_heading');
			} else {
				$data['text_related_product'] = $this->language->get('text_related_product');
			}

			if($this->config->has('config_simple_comment_related_heading')) {
				$data['text_related_comment'] = $this->config->get('config_simple_comment_related_heading');
			} else {
			...
			if($article_info) {
				...
				if($this->config->has('config_simple_blog_related_articles')) {
					$data['simple_blog_related_articles'] = $this->config->get('config_simple_blog_related_articles');
				}

				if($this->config->has('config_simple_blog_author_information')) {
					$data['simple_blog_author_information'] = $this->config->get('config_simple_blog_author_information');
				}

				if($this->config->has('config_simple_blog_share_social_site')) {
					$data['simple_blog_share_social_site'] = $this->config->get('config_simple_blog_share_social_site');
				}
			...
			$data['breadcrumbs'][] = array(
				...
				'href'      => $this->url->link('simple_blog/article/view', 'simple_blog_article_id=' . $simple_blog_article_id, 'SSL'),
				'separator' => ' :: '
				);
	...
	public function writeComment() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				...
				if($this->config->get('config_simple_blog_comment_auto_approval')) {
