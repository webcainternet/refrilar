<?php
$_['heading_title']    = 'Google Analytics';

// Text
$_['text_analytics']   = 'Estatísticas';
$_['text_success']	   = 'Google Analytics modificado com sucesso!';
$_['text_edit']        = 'Editando Google Analytics';
$_['text_signup']      = 'Acesse o site <a href="http://www.google.com/analytics/" target="_blank"><u>Google Analytics</u></a> e registre seu domínio para gerar o código de integração.';

// Entry
$_['entry_code']       = 'Código do Google Analytics';
$_['entry_status']     = 'Situação';

// Error
$_['error_permission'] = 'Atenção: Você não possui permissão para modificar a extensão de estatística Google Analytics!';
$_['error_code']	   = 'O código do Google Analytics é obrigatório!';
